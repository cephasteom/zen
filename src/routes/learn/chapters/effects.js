...
Track FX are applied to the output of a track. By default, each stream is routed to its own track, with \`s0\` using track 0 (channels 1 and 2), \`s1\` using track 1 (channels 3 and 4), and so on. You can change the track a stream is routed to by setting the \`track\` parameter. Each track has a chain of effects available, including reverb, delay, distortion and high and low cut filters. For efficiency, these are only instantiated when you use them, so you might hear a small artifact whilst they are being created.
\`\`\`js
...
Of course, using separate reverbs, delays, etc. for every track is expensive and may have performance implications, depending on your machine. FX Streams are separate streams for controlling your effects. They like any other Stream and are used to create interesting patterns. FX Streams are represented by the variables \`fx0\`, \`fx1\`, etc. To route a stream to an FX Stream use the variable name as a parameter. The following example routes \`s0\` to \`fx0\`:
\`\`\`js
